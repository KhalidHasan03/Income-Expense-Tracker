...
<div><h1 class="text-2xl font-semibold tracking-tight">Categories</h1><p class="text-sm text-zinc-500">You create • {{ $stats['total'] }} total • <span class="text-emerald-600">{{ $stats['income'] }} income</span> • <span class="text-red-600">{{ $stats['expense'] }} expense</span></p></div>
...
<div class="min-w-0"><div class="font-medium text-sm truncate">{{ $c->name }} <span class="ml-2 text-xs px-2 py-0.5 rounded-full {{ $c->type=='income'?'bg-emerald-100 text-emerald-700':'bg-red-100 text-red-700' }}">{{ $c->type }}</span></div><div class="text-xs text-zinc-500">{{ $c->transactions_count }} transactions • {{ $c->created_at->diffForHumans() }}</div></div></div>
<div class="flex gap-1 shrink-0">
<button onclick="openCatEdit({{ $c->id }})" class="btn btn-ghost !px-3 !py-1.5 text-xs">Edit</button>
<form method="POST" action="{{ route('categories.destroy',$c) }}" onsubmit="return confirm('Delete {{ $c->name }}? Only if no transactions.')">@csrf @method('DELETE')<button class="btn btn-ghost !px-3 !py-1.5 text-xs text-red-600">Delete</button></form>
</div>
</div>
...
<div class="flex justify-end gap-2"><button type="button" onclick="this.closest('dialog').close()" class="btn btn-ghost">Cancel</button><button class="btn btn-primary">Save</button></div>
</form>
</dialog>
@endforeach
</div>
